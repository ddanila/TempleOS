...
`Kernel/I386/TaskContext.HC` switches between ring-0 stacks in the same flat address
space. Its native ABI is `U0 switch(CI386Context *old, CI386Context *next)`, with
two eight-byte HolyC argument slots. The source builds as a relocatable HolyC
assembly module alongside idle and segment reload entries. The bootstrap embeds
its generated code and passes the exported switch address as a function pointer.
Full integration with the public TempleOS `Yield` implementation remains pending.
...
`i386_idle` in `Kernel/I386/TaskContext.HC` provides a four-instruction `U0 idle()` primitive:
...
primitive returns with IF clear. The bootstrap uses the HolyC-generated entry.
...
and masks IRQs while editing it. `i386_segments_reload` in `TaskContext.HC` reloads FS/GS from two U16 selector
...
## HolyC assembly build

The task, input, message and exception-task fixtures compile `TaskContext.HC`
inside TempleOS and export `TaskContext.t32m`. The host requires exactly the
three expected code exports and no relocations, then embeds the generated bytes
and derives entry boundaries from export offsets. Each entry remains covered by
the native instruction audit; checked zero padding after the final return is
excluded. The surrounding boot/IRQ runner still uses NASM, and the compiler host
is still x86-64. This migration alone does not establish native self-hosting.
