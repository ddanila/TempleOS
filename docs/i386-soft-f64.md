...
`I386F64Sub(U64 a,U64 b)`. Arguments and return values are binary64 **bit
...
normal/subnormal boundaries and carry into the exponent.
...
and compares both addition and subtraction bit-for-bit against host binary64
arithmetic, with explicit expected NaN propagation. It includes a cross-product
of special/boundary values, selected alignment distances, halfway cases,
cancellation and deterministic random inputs. The host patches only the exported
