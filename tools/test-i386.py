...
def main():
    ...
    modes.add_argument('--irq', action='store_true', help='Test native PIC/PIT/RTC interrupts and register/flag restoration')
    args = parser.parse_args()
    kind = 'expressions'
    for mode in ('functions', 'data', 'vga', 'heap', 'memory', 'a20', 'irq'):
        if getattr(args, mode):
            kind = mode
    data_mode = kind in ('data', 'vga', 'heap', 'memory', 'a20', 'irq')
    ...
    while True:
        ...
        offset += size
        count += 1
    if offset != len(data) or count != {'data': 16, 'functions': 150, 'expressions': 9}.get(kind, 1):
    ...
    disk = OUT/'runner.img'
    run('nasm', *(['-DIRQ_TEST=1'] if args.irq else []), *(['-DVGA_TEST=1'] if args.vga else []), *(['-DFUNCTIONS=1'] if functions else []),
    ...
    (OUT/'result.json').write_text(json.dumps({'cases': count, 'cpu': '486',
        ...
        'ram_mib': 8, 'fault_cases': 4 if functions and not data_mode else 0, 'result': 'pass', 'scope': 'PIC/PIT/RTC interrupts and frame restoration' if args.irq else 'A20 methods and extended-memory allocation' if args.a20 else 'BIOS memory handoff and arena selection' if args.memory else 'native arena heap' if args.heap else f'integer {kind} backend'}, indent=2)+'\n')
