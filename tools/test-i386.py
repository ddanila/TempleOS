...
def main():
    ...
    modes.add_argument('--irq', action='store_true', help='Test native PIC/PIT/RTC interrupts, exceptions and frame restoration')
    modes.add_argument('--tasks', action='store_true', help='Test cooperative native task contexts and separate stacks')
    args = parser.parse_args()
    kind = 'expressions'
    for mode in ('functions', 'data', 'vga', 'heap', 'memory', 'a20', 'irq', 'tasks'):
        if getattr(args, mode):
            kind = mode
    data_mode = kind in ('data', 'vga', 'heap', 'memory', 'a20', 'irq', 'tasks')
    ...
    (OUT/'result.json').write_text(json.dumps({'cases': count, 'cpu': '486',
        ...
        'ram_mib': 8, 'fault_cases': 4 if functions and not data_mode else 0, 'result': 'pass', 'recovered_exceptions': 3 if args.irq else 0, 'scope': 'cooperative task contexts' if args.tasks else 'PIC/PIT/RTC interrupts, exceptions and frame restoration' if args.irq else 'A20 methods and extended-memory allocation' if args.a20 else 'BIOS memory handoff and arena selection' if args.memory else 'native arena heap' if args.heap else f'integer {kind} backend'}, indent=2)+'\n')
