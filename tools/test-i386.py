...
def main():
    ...
    boot_sectors = 768 if args.task_symbols else 640 if args.lex_define or args.lex_tokens else 512 if args.lex_state or args.ata_tasks or args.redsea_read or args.arc_expand or args.lex_number or args.lex_tokens or args.lex_define or args.lex_cond else 384 if args.tasks or args.task_heaps or args.heap else 320 if args.float or args.redsea or args.redsea_bind or args.ata_tasks or args.tasks or args.input or args.messages or args.except_tasks or args.symbols else 256 if large_runner or args.heap or args.except_records or except_runner else 128
    kind = 'expressions'
    ...
    disk = OUT/'runner.img'
    run('nasm', *(['-DLEX_MACRO_TEST=1'] if args.lex_define or args.lex_tokens else []), *(['-DCOMPILER_GRAPH_TEST=1'] if args.task_symbols else []), *(['-DLARGE_CORE_TEST=1'] if args.tasks or args.task_heaps or args.heap else []), *(['-DLEX_LARGE_TEST=1'] if args.lex_state or args.ata_tasks or args.redsea_read or args.arc_expand or args.lex_number or args.lex_tokens or args.lex_define or args.lex_cond else []), *(['-DEXCEPT_TASK_TEST=1'] if args.except_tasks else []), *(['-DEXCEPT_CONTEXT_TEST=1'] if except_runner else []), *(['-DSOFT_F64_TEST=1'] if args.lex_cond or args.lex_define or args.lex_tokens or args.lex_number or args.soft_f64_log or args.soft_f64_unary or args.soft_f64 or args.soft_f64_convert or args.soft_f64_compare or args.soft_f64_to_int or args.float else []), f'-DBOOT_SECTORS={boot_sectors}', *(['-DTASK_TEST=1'] if task_runner else []), *(['-DIRQ_TEST=1'] if args.irq else []), *(['-DVGA_TEST=1'] if args.vga else []), *(['-DFUNCTIONS=1'] if functions else []),
