...
def main():
    ...
    disk = OUT/'runner.img'
    run('nasm', *(['-DBOOT_SECTORS=256'] if args.tasks or args.irq else []), *(['-DTASK_TEST=1'] if args.tasks else []), *(['-DIRQ_TEST=1'] if args.irq else []), *(['-DVGA_TEST=1'] if args.vga else []), *(['-DFUNCTIONS=1'] if functions else []),
    ...
    if disk.stat().st_size > (257 if args.tasks or args.irq else 129)*512:
    ...
    (OUT/'result.json').write_text(json.dumps({'cases': count, 'cpu': '486',
        ...
        'ram_mib': 8, 'fault_cases': 4 if functions and not data_mode else 0, 'result': 'pass', 'recovered_exceptions': 3 if args.irq else 0, 'scope': 'cooperative task contexts' if args.tasks else 'PIC/PIT/RTC/keyboard interrupts, input queue, exceptions and frame restoration' if args.irq else 'A20 methods and extended-memory allocation' if args.a20 else 'BIOS memory handoff and arena selection' if args.memory else 'native arena heap' if args.heap else f'integer {kind} backend'}, indent=2)+'\n')
