...
def main():
    ...
    modes.add_argument('--ata', action='store_true', help='Test native ATA identify and single-sector PIO reads')
    ...
    if args.ata:
        with disk.open('r+b') as stream:
            for lba in (2048, 2049, 32767):
                stream.seek(lba*512)
                stream.write(bytes(((i*37)^(i>>1)^(lba&255))&255 for i in range(512)))
    if args.vga:
    ...
    (OUT/'result.json').write_text(json.dumps({'cases': count, 'cpu': '486',
        ...
        'ram_mib': 8, 'fault_cases': 4 if functions and not data_mode else 0, 'result': 'pass', 'recovered_exceptions': 3 if args.irq else 0, 'scope': 'ATA identify and 16-bit PIO sector reads' if args.ata else 'keyboard broker and task message delivery' if args.messages else 'blocking keyboard input and IRQ-driven task wakeups' if args.input else 'cooperative task contexts' if args.tasks else 'PIC/PIT/RTC/keyboard interrupts, input queue, exceptions and frame restoration' if args.irq else 'A20 methods and extended-memory allocation' if args.a20 else 'BIOS memory handoff and arena selection' if args.memory else 'native arena heap' if args.heap else f'integer {kind} backend'}, indent=2)+'\n')
