...
def main():
    ...
    modes.add_argument('--keywords', action='store_true', help='Test owned native language and assembler keywords')
    modes.add_argument('--lex-cond', action='store_true', help='Test native conditional preprocessing and skipped branches')
    modes.add_argument('--lex-define', action='store_true', help='Test native definition reading, publication and expansion')
    modes.add_argument('--lex-tokens', action='store_true', help='Test mixed native token streams and macro dispatch')
    modes.add_argument('--lex-ident', action='store_true', help='Test identifier scanning and local/global lookup')
    modes.add_argument('--lex-punct', action='store_true', help='Test operator/comment parsing against the production lexer')
    ...
    modes.add_argument('--arc', action='store_true', help='Test shared compression dictionary growth and reuse')
    ...
    modes.add_argument('--ata-tasks', action='store_true', help='Test cooperative ATA transactions across two drives and channel poisoning')
    modes.add_argument('--task-symbols', action='store_true', help='Test task-owned symbol scopes and parent lifetime')
    modes.add_argument('--task-heaps', action='store_true', help='Test public task heap ownership, cleanup order and rollback')
    ...
    boot_sectors = 768 if args.task_symbols else 640 if args.lex_define or args.lex_tokens else 512 if args.lex_state or args.ata_tasks or args.redsea_read or args.arc_expand or args.lex_number or args.lex_tokens or args.lex_define or args.lex_cond else 384 if args.tasks or args.task_heaps else 320 if args.float or args.redsea or args.redsea_bind or args.ata_tasks or args.tasks or args.input or args.messages or args.except_tasks or args.symbols else 256 if large_runner or args.heap or args.except_records or except_runner else 128
    kind = 'expressions'
    ...
    if args.lex_number:
        run(sys.executable, 'tools/gen-i386-lex-number.py', '--check')
        oracle = json.loads((ROOT/'tests/i386/lex-number-values.json').read_text())
        cases = ROOT/'tests/guest/i386-lex-number/Cases.HC'
        if hashlib.sha256(cases.read_bytes()).hexdigest() != oracle['cases_sha256']:
            raise ValueError('Numeric cases no longer match the original x64 oracle')
    ...
    disk = OUT/'runner.img'
    run('nasm', *(['-DLEX_MACRO_TEST=1'] if args.lex_define or args.lex_tokens else []), *(['-DCOMPILER_GRAPH_TEST=1'] if args.task_symbols else []), *(['-DLARGE_CORE_TEST=1'] if args.tasks or args.task_heaps else []), *(['-DLEX_LARGE_TEST=1'] if args.lex_state or args.ata_tasks or args.redsea_read or args.arc_expand or args.lex_number or args.lex_tokens or args.lex_define or args.lex_cond else []), *(['-DEXCEPT_TASK_TEST=1'] if args.except_tasks else []), *(['-DEXCEPT_CONTEXT_TEST=1'] if except_runner else []), *(['-DSOFT_F64_TEST=1'] if args.lex_cond or args.lex_define or args.lex_tokens or args.lex_number or args.soft_f64_log or args.soft_f64_unary or args.soft_f64 or args.soft_f64_convert or args.soft_f64_compare or args.soft_f64_to_int or args.float else []), f'-DBOOT_SECTORS={boot_sectors}', *(['-DTASK_TEST=1'] if task_runner else []), *(['-DIRQ_TEST=1'] if args.irq else []), *(['-DVGA_TEST=1'] if args.vga else []), *(['-DFUNCTIONS=1'] if functions else []),
        ...
        if args.redsea_load_set or args.redsea_bind:
            ...
            with disk.open('r+b') as stream:
                ...
                for block, content in ((2060, consumer), (2064, provider), (2068, wrong),
                    ...
                    stream.write(content)
        if args.redsea_read:
            #A second independently mounted volume proves drive routing for compiler input.
            second_source = b'I64 OtherDrive() { return 32; }\n'
            with disk.open('r+b') as stream:
                stream.seek(3100*512)
                stream.write(rs_entry('.', 0x810, 3100, 512) +
                             rs_entry('..', 0x810, 3100, 0) +
                             rs_entry('Source.HC', 0x800, 3101, len(second_source)) + bytes(320))
                stream.seek(3101*512)
                stream.write(second_source)
    ...
    if args.ata_tasks:
        cmd += ['-drive', f'file={slave},format=raw,if=ide,index=1']
    if args.ata or args.ata_tasks or args.redsea_create or args.redsea_delete or args.redsea_replace:
