...
def main():
    ...
    kind = 'expressions'
    for mode in ('functions', 'data', 'vga', 'heap', 'memory', 'a20'):
        if getattr(args, mode):
            kind = mode
    data_mode = kind in ('data', 'vga', 'heap', 'memory', 'a20')
    ...
    (OUT/'result.json').write_text(json.dumps({'cases': count, 'cpu': '486',
        ...
        'ram_mib': 8, 'fault_cases': 4 if functions and not data_mode else 0, 'result': 'pass', 'scope': 'A20 methods and extended-memory allocation' if args.a20 else 'BIOS memory handoff and arena selection' if args.memory else 'native arena heap' if args.heap else f'integer {kind} backend'}, indent=2)+'\n')
