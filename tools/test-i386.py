...
def main():
    ...
    modes.add_argument('--soft-f64-unary', action='store_true', help='Test software F64 Abs/Sqr against x64 and a host oracle')
    ...
    if args.soft_f64_unary or args.integer_math or args.soft_f64 or args.soft_f64_convert or args.soft_f64_compare or args.soft_f64_to_int:
        ...
        oracle = (make_unary_oracle(1024) if args.soft_f64_unary else make_integer_math_oracle() if args.integer_math else make_to_int_oracle(1024) if args.soft_f64_to_int else
        ...
        if args.soft_f64_unary and (exports/'x64-unary.bin').read_bytes() != oracle:
            raise ValueError('x64 Abs/Sqr differs from the unary oracle')
    ...
    disk = OUT/'runner.img'
    run('nasm', *(['-DSOFT_F64_TEST=1'] if args.soft_f64_unary or args.soft_f64 or args.soft_f64_convert or args.soft_f64_compare or args.soft_f64_to_int or args.float else []), *(['-DBOOT_SECTORS=256'] if large_runner else []), *(['-DTASK_TEST=1'] if task_runner else []), *(['-DIRQ_TEST=1'] if args.irq else []), *(['-DVGA_TEST=1'] if args.vga else []), *(['-DFUNCTIONS=1'] if functions else []),
