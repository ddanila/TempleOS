...
def compiler_runtime_layout(module):
    ...
    if version_offset+4 > 32+size or struct.unpack_from('<I', module, version_offset)[0] != 46:
...
def verify_compiler_rejection(disk, volume, out, layout):
    ...
    for label, position, replacement, reason in (
            ('wrong-target', 6, b'\x04', 'load'),
            ('missing-import', layout['import_offset'], b'X', 'load'),
            ('wrong-api', layout['version_offset'], struct.pack('<I', 45), 'api')):
...
def main():
    ...
    if args.test:
        ...
        result['compiler_runtime'] = dict(module='CompilerRuntime', version=46, image_address=address,
