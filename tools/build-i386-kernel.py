...
def compiler_runtime_layout(module):
    ...
    if version_offset+4 > 32+size or struct.unpack_from('<I', module, version_offset)[0] != 39:
...
def verify_compiler_rejection(disk, volume, out, layout):
    ...
    for label, position, replacement, reason in (
            ('wrong-target', 6, b'\x04', 'load'),
            ('missing-import', layout['import_offset'], b'X', 'load'),
            ('wrong-api', layout['version_offset'], struct.pack('<I', 38), 'api')):
...
def main():
    ...
    if args.test:
        ...
        result['file_runtime'] = dict(version=17, image_address=file_address, image_bytes=file_size,
        ...
        result['compiler_probe'] = dict(module='CompilerProbe', version=9, image_address=probe_address,
        ...
        result['compiler_runtime'] = dict(module='CompilerRuntime', version=39, image_address=address,
        ...
        result['console_runtime']=dict(version=5,image_bytes=csize,retained_heap_bytes=cspan,
