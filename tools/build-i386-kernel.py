...
def compiler_runtime_layout(module):
    ...
    if version_offset+4 > 32+size or struct.unpack_from('<I', module, version_offset)[0] != 57:
...
def verify_native_leaf_module(disk):
    """Check the guest-persisted dependency used in a two-file native link."""
    path='/Probe/DurableLeaf.t32m'
    module=mutated_file_contents(disk,{path}).get(path)
    if not module or len(module)<32 or module[:4]!=b'T32M':
        raise ValueError('Missing guest-built leaf module')
    total,code_size,count,records_offset,strings_offset=struct.unpack_from('<5I',module,12)
    if (total!=len(module) or not code_size or code_size&7 or count!=1 or
            records_offset!=32+code_size or strings_offset!=records_offset+16):
        raise ValueError('Invalid guest-built leaf module layout')
    kind,offset,name,length=struct.unpack_from('<4I',module,records_offset)
    if (kind!=1 or offset!=0 or module[name:name+length]!=b'DurableLeaf' or
            name<strings_offset or name+length>=total or module[name+length]):
        raise ValueError('Guest-built leaf module lost its export')
...
def verify_native_owned_data_module(disk):
    """Audit a guest-built module containing both code and scalar storage."""
    path='/Probe/DurableOwned.t32m'
    module=mutated_file_contents(disk,{path}).get(path)
    if not module or len(module)<32 or module[:4]!=b'T32M':
        raise ValueError('Missing guest-built owned-data module')
    total,code_size,count,records_offset,strings_offset=struct.unpack_from('<5I',module,12)
    if (total!=len(module) or not code_size or code_size&7 or count!=4 or
            records_offset!=32+code_size or strings_offset!=records_offset+64 or
            strings_offset>total):
        raise ValueError('Invalid guest-built owned-data module layout')
    records=[struct.unpack_from('<4I',module,records_offset+16*i) for i in range(count)]
    function,address,data_range,data_export=records
    if (function[0]!=1 or function[1]!=0 or
            module[function[2]:function[2]+function[3]]!=b'DurableLater' or
            address[0]!=5 or address[1]<1 or address[1]>code_size-4 or
            module[address[2]:address[2]+address[3]]!=b'DurableData' or
            module[32+address[1]-1]!=0x05 or
            module[32+address[1]:32+address[1]+4]!=b'\0'*4 or
            data_range[0]!=4 or data_range[2]!=8 or data_range[3]!=0 or
            data_export[0]!=3 or data_export[1]!=data_range[1] or
            module[data_export[2]:data_export[2]+data_export[3]]!=b'DurableData' or
            data_range[1]>code_size-8 or
            module[32+data_range[1]:32+data_range[1]+8]!=struct.pack('<q',20)):
        raise ValueError('Guest-built owned-data records or initial value differ')
    return {'sha256':hashlib.sha256(module).hexdigest(),
            'function':'DurableLater','global':'DurableData',
...
def verify_native_unit_module(disk):
    """Audit every definition and relocation in the guest-built source unit."""
...
def main():
    ...
    if args.test:
        ...
        native_split = [tuple(int(value,16) for value in line.split()[2:])
                        for line in log.splitlines()
                        if re.match(r'^NATIVE SPLIT [0-9A-F]{16} ',line)]
        if len(native_split)!=2 or [entry[0] for entry in native_split]!=[0,1] or \
                native_split[0][1:]!=native_split[1][1:] or \
                native_split[0][1]<80 or native_split[0][2]<50 or \
                native_split[0][3]<80:
            raise ValueError('Guest-built modules did not cross-link')
        result['native_split_link']={'phases':[0,1],
                                     'root_module_bytes':native_split[0][1],
        ...
        native_data_owned = [tuple(int(value,16) for value in line.split()[3:])
                             for line in log.splitlines()
                             if re.match(r'^NATIVE DATA OWNED [0-9A-F]{16} ',line)]
        if len(native_data_owned)!=2 or [entry[0] for entry in native_data_owned]!=[0,1] or \
                native_data_owned[0][1:]!=native_data_owned[1][1:] or \
                native_data_owned[0][1]<120 or native_data_owned[0][2]<48:
            raise ValueError('Guest-built module did not own mutable global data')
        result['native_owned_global']={'phases':[0,1],
        ...
        native_unit = [tuple(int(value,16) for value in line.split()[2:])
                       for line in log.splitlines()
                       if re.match(r'^NATIVE UNIT [0-9A-F]{16} ',line)]
        if len(native_unit)!=2 or [entry[0] for entry in native_unit]!=[0,1] or \
                native_unit[0][1:]!=native_unit[1][1:] or \
                native_unit[0][1]<128 or native_unit[0][2]<48:
            raise ValueError('Guest-built source unit did not load and execute')
        ...
        native_alloc = [tuple(int(value,16) for value in line.split()[2:])
                        for line in log.splitlines()
                        if re.match(r'^NATIVE ALLOC [0-9A-F]{16} ',line)]
        if len(native_alloc)!=2 or [entry[0] for entry in native_alloc]!=[0,1] or \
                native_alloc[0][1:]!=native_alloc[1][1:] or \
                native_alloc[0][1]<256 or native_alloc[0][2]<128:
            raise ValueError('Guest-built original module allocator did not execute')
        result['native_alloc']={'phases':[0,1],
        ...
        result['native_bound_disk']={'path':'C:/Probe/DurableBound.t32m',
                                     ...
                                     'result':'pass'}
        result['native_split_disk']={'paths':['C:/Probe/DurableBound.t32m',
                                              'C:/Probe/DurableLeaf.t32m'],
                                     'fresh_boot':'write, read, cross-link, execute',
        ...
        result['native_data_bound_disk']={'path':'C:/Probe/DurableDataRead.t32m',
                                          ...
                                          'result':'pass'}
        result['native_owned_global_disk']={'path':'C:/Probe/DurableOwned.t32m',
        ...
        result['native_named_pointer_disk']={'paths':['C:/Probe/DurableNamedConsumer.t32m',
                                             ...
                                             'result':'pass'}
        result['native_unit_disk']={'path':'C:/Probe/DurableUnit.t32m',
                                    'fresh_boot':'write, read, load, mutate, execute',
                                    'second_boot':'read, load, mutate, execute, replace',
        ...
        result['native_loader_disk']={'path':'C:/Probe/NativeLoader.t32m',
                                      ...
                                      'result':'pass'}
        result['native_alloc_disk']={'path':'C:/Probe/NativeAlloc.t32m',
                                     'fresh_boot':'write, load, execute',
                                     'second_boot':'read, load, execute, replace',
